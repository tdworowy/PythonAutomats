...
def get_titles_for_user(user):
    ...
    last_page = 2
    titles_map = map(
        get_titles, [url + "?page= %s" % str(i) for i in range(1, last_page)]
    )
...
task_tag_songs = PythonOperator(
    task_id="Tag_songs", provide_context=True, python_callable=tag_songs, dag=dag
)
