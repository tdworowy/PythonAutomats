...
class FacebookBot():
    ...
    def post(self, content, vis):
        post_filed = self.driver.find_element_by_css_selector("div[role='presentation']")
        post_filed.click()
        post_filed.send_keys(content)

        self.set_visibility(vis)

        post_button = self.driver.find_element_by_css_selector("button[data-testid='react-composer-post-button'")
        post_button.click()
